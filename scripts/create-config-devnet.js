...
const DEPLOY_FILE = path.join(__dirname, '..', 'deployments', 'devnet-config.json');
...
async function main() {
  if (SEND && fs.existsSync(DEPLOY_FILE)) {
    throw new Error('Config already created. See deployments/devnet-config.json');
  }
  ...
  console.log('Saved to : deployments/devnet-config.json');
}
